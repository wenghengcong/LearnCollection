...
/// 解法：https://leetcode.cn/problems/happy-number/solutions/224894/kuai-le-shu-by-leetcode-solution/
///
/// 一个“快乐数”定义为：对于一个正整数，每一次将该数替换为它每个位置上的数字的平方和，
/// 然后重复这个过程直到这个数变为 1，也可能是无限循环但始终变不到 1。
/// 如果可以变为 1，那么这个数就是快乐数。
class E_202_HappyNumber {
   class func getNext(_ n: Int) -> Int {
        var sum = 0
        var nC = n
        while nC > 0 {
            let lastNum = nC % 10
    ...
    /*
     2. 方法：使用“快慢指针”思想找出循环：“快指针”每次走两步，“慢指针”每次走一步，
     ...
     */
    class func isHappy_2(_ n: Int) -> Bool {
        var slow = n
        var fast = getNext(n)
