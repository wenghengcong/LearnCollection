...
{
    //iOS 11,12，在Alert、Badge、Sound关闭时，返回0
    //3-8，在8以上系统该值不能获取，总是返回UIUserNotificationTypeNone
   UIUserNotificationType typesOf3_8 = [[UIApplication sharedApplication] enabledRemoteNotificationTypes];
    //8-10
    UIUserNotificationType typesOf8_10 = [[UIApplication sharedApplication] currentUserNotificationSettings].types;
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 10.0) {
        ...
        [center getNotificationSettingsWithCompletionHandler:^(UNNotificationSettings * _Nonnull settings) {
            if (settings) {
                NSLog(@"%@", settings);
                //1.  __IOS_AVAILABLE(10.0)
                //UNNotificationSettingNotSupported、UNNotificationSettingDisabled、UNNotificationSettingEnabled
                UNAuthorizationStatus authorizationStatus = settings.authorizationStatus;
                UNNotificationSetting soundSetting = settings.soundSetting;
                UNNotificationSetting badgeSetting = settings.badgeSetting;
                //enable 时，才有alertStyle
                UNNotificationSetting alertSetting = settings.alertSetting;
                UNNotificationSetting notificationCenterSetting = settings.notificationCenterSetting;
                UNNotificationSetting lockScreenSetting = settings.lockScreenSetting;
                //该项不支持
                UNNotificationSetting carPlaySetting = settings.carPlaySetting;

                //UNAlertStyleNone、UNAlertStyleBanner、UNAlertStyleAlert
                UNAlertStyle alertStyle = settings.alertStyle;

                //2. __IOS_AVAILABLE(11.0) 是否显示预览，始终、解锁时、从不
                //UNShowPreviewsSettingAlways、UNShowPreviewsSettingWhenAuthenticated、UNShowPreviewsSettingNever
                if (@available(iOS 11.0, *)) {
                    UNShowPreviewsSetting showPreviewsSetting = settings.showPreviewsSetting;
                    NSLog(@"showPreviews: %ld", (long)showPreviewsSetting);
                }

                //3. __IOS_AVAILABLE(12.0)
                if (@available(iOS 12.0, *)) {
                    UNNotificationSetting criticalAlertSetting = settings.criticalAlertSetting;
                    //是否提供了自定义通知的界面
                    BOOL providesAppNotificationSettings = settings.providesAppNotificationSettings;
                    NSLog(@"critical-%ld, providesAppNotificationSettings-%d",
                          (long)criticalAlertSetting, providesAppNotificationSettings);
                }
                NSLog(@"%@", settings);
            }
        }];
    ...
    NSNumber *allowPush = (pushOption > 0) ? @1 : @0;
}
